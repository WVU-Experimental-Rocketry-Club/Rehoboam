        ...
        if ctx.guild:
            ...
            if ctx.author.bot:
                ...
                for embed in ctx.embeds:
                    if any(re.search(site, embed.url, re.IGNORECASE) is not None for site in whitelist):
                        if any(re.search(site, embed.url, re.IGNORECASE) is not None for site in blacklist):
                            return
                        else:
                            await ctx.channel.send(embed.url)
                    else:
                        return
                else:
                    return
